...
const {con, register, login, writeBoard, list, query,movieList} = require('./db/DB');
...
app.get('/movie', (req,res) => {
    ...
    const client_id = 'LDV_m769LOrkxbVEMdtM';
    let client_secret = 'PqET_ZqXO6';
    var api_url = `https://openapi.naver.com/v1/search/movie.json?query=${encodeURI(req.query.word)}&display=100&genre=${encodeURI(req.query.genre)}`; // json 결과
    // request.get('주소','옵션','콜백')
    ...
    request.get(options, (error, response, body) => {
        if (!error && response.statusCode == 200) {
            ...
            res.render('movie/search', {list});
            list.forEach(item => {
                item.genre = req.query.genre;
                movieList(req,res,item);
            });
        } else {
